...
def configuration_page():
    ...
    with tab2:
        ...
        with st.expander("Ajouter une entreprise"):
            with st.form("add_company"):
                ...
                if st.form_submit_button("Ajouter"):
                    companies_file = COMPANIES_DIR / "companies.parquet"
                    companies_df = pd.read_parquet(companies_file) if companies_file.exists() else pd.DataFrame()
                    ...
                    companies_df.to_parquet(companies_file)
                    st.success("Entreprise ajoutée")
                    st.rerun()
    ...
    with tab3:
        ...
        with col1:
            st.write("**Paramètres de calcul**")
            plafond_ss = st.number_input("Plafond Sécurité Sociale", value=3428.0)
            smic_horaire = st.number_input("SMIC horaire", value=11.65)
            
        with col2:
            st.write("**Taux de charges**")
            st.write("Ces paramètres seront utilisés pour les calculs automatiques")
            taux_charges_sal = st.slider("Taux charges salariales (%)", 0, 50, 22)
            taux_charges_pat = st.slider("Taux charges patronales (%)", 0, 50, 35)
